...
class FullTeleop {
    ...
    val myFirstMercurialTeleOp: RegisterableProgram = teleop { ctx: Context? ->
        ...
        val turret = Turret(ctx.hardwareMap, Constants.AllianceColors.BLUE, follower)
        val spindexer = Spindexer(ctx.hardwareMap)
        val flywheel = Flywheel(ctx.hardwareMap, spindexer)
        ...
        ctx.bindSpawn(
            ctx.risingEdge { !ctx.gamepad2.dpad_left },
            exec {
                spindexer.getPurpleAction()?.let { spindexer.spin.tx.send(it) }
            })

        ctx.bindSpawn(
            ctx.risingEdge { !ctx.gamepad2.dpad_right },
            exec {
